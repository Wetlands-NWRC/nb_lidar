...
LOOKUP_FILE = Path(__file__).parent.parent.parent / Path("references/lookup.json")
...
def build_confusion_matrix(data: dict[str, Any], lables: str = None) -> pd.DataFrame:
    ...
    table = init_confusion_matrix(
        array=data.get('confusion_matrix'),
        labels=lables
    )
    table = add_producers(
        df=table,
        array=data.get('producers')
    )
    table = add_consumers(
        df=table,
        array=data.get('consumers')
    )
    table = add_overall(
        df=table,
        data=data.get('overall')
    )
    return table
