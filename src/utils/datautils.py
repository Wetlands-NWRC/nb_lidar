...
def classification_to_cloud_stroage(image, bucket, file_name, region, start: bool = False) -> None:
...
def confusion2collection(confusion_matrix) -> ee.FeatureCollection:
    features = [
        ...
        ee.Feature(None, {'producers': confusion_matrix.producersAccuracy().toList().flatten()}),
        ee.Feature(None, {'consumers': confusion_matrix.consumersAccuracy().toList().flatten()}),
...
def confusion_matrix_to_drive(confusion_matrix, folder, name, start: bool = True) -> ee.batch.Task:
